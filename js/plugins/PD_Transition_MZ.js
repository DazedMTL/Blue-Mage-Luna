...
(function () {
  ...
  Sprite_Transition.prototype.createTransitionData = function (fileName) {
    ...
    if (fileName) {
      //ファイル名が設定されていたら読み込む
      var bitmap2 = ImageManager.loadTransition(
        $gameSystem.getTransitionFileName()
      );
  ...
  Sprite_Transition.prototype.updateBitmap = function (duration, sign) {
    if ($gameSystem.isTransitionBinarization()) {
      ...
      if (sign > 0) {
        ...
        this.bitmap.createTransitionBinarizationFadeIn(
        ...
        this.bitmap.createTransitionBinarizationFadeOut(
          this._transitionData,
          threshold,
          this._white
        );
      ...
      if (sign > 0) {
        ...
        this.bitmap.createTransitionFadeIn(
          this._transitionData,
          threshold,
          speed
        );
        ...
        this.bitmap.createTransitionFadeOut(
          this._transitionData,
          threshold,
          speed,
          this._white
  ...
  Sprite_Transition.prototype.setDefault = function (sign) {
    if (this._transitionBitmap) {
      ...
      var bitmap = new Bitmap(Graphics.width, Graphics.height);
      bitmap.blt(
        this._transitionBitmap,
        0,
        0,
        this._transitionBitmap.width,
        this._transitionBitmap.height,
        0,
        0,
        bitmap.width,
        bitmap.height
  ...
  Game_Interpreter.prototype.command221 = function () {
    if (!$gameMessage.isBusy()) {
      SceneManager._scene.startFadeOut(
        $gameSystem.getTransitionDuration() || PD_durationTime
      );
  ...
  //トランジションデータを作る
  Bitmap.prototype.createTransitionBinarizationFadeIn = function (
    data,
    threshold
  ) {
  ...
  Bitmap.prototype.createTransitionBinarizationFadeOut = function (
    data,
    threshold,
    white
  ) {
  ...
  Bitmap.prototype.createTransitionFadeOut = function (
    data,
    threshold,
    speed,
    white
