...
Moghunter.b_motion_shake_effect_actor = String(
  Moghunter.parameters["Damage Motion Actor"] || "true"
);
Moghunter.b_motion_shake_effect_enemy = String(
  Moghunter.parameters["Damage Motion Enemy"] || "true"
);
Moghunter.b_motion_disable_blink_damage = String(
  Moghunter.parameters["Disable Blink Damage"] || "true"
);
Moghunter.b_motion_shadow = String(
  Moghunter.parameters["Shadow (Float Motion)"] || "true"
);
Moghunter.b_motion_shadowOpacity = Number(
  Moghunter.parameters["Shadow Opacity"] || 170
);
Moghunter.b_motion_shadowZoom = String(
  Moghunter.parameters["Shadow Zoom Effect"] || "true"
);
Moghunter.b_motion_defaultDamage = Number(
  Moghunter.parameters["Default Damage Motion"] || 0
);
Moghunter.b_motion_actorAction = String(
  Moghunter.parameters["Actor Action Motion"] || "false"
);
Moghunter.b_motion_enemyAction = String(
  Moghunter.parameters["Enemy Action Motion"] || "true"
);
Moghunter.b_motion_groundHeight = Number(
  Moghunter.parameters["Battleback Ground Height"] || 200
);
...
Game_System.prototype.initialize = function () {
  ...
  if (String(Moghunter.b_motion_shake_effect_actor) == "true") {
    this._bmotion[0] = true;
  }
  if (String(Moghunter.b_motion_shake_effect_enemy) == "true") {
    this._bmotion[1] = true;
  }
  if (String(Moghunter.b_motion_actorAction) == "true") {
    this._bmotion[2] = true;
  }
  if (String(Moghunter.b_motion_enemyAction) == "true") {
    this._bmotion[3] = true;
  }
};
...
Game_Action.prototype.canBmotionAction = function (item) {
  if (this.subject().isActor() && !$gameSystem._bmotion[2]) {
    return false;
  }
  if (this.subject().isEnemy() && !$gameSystem._bmotion[3]) {
    return false;
  }
  return true;
};
...
Game_Action.prototype.needSetBmotionDamage = function (target, oldhp) {
  if (target._bmotion.damage.mode == -1) {
    return false;
  }
  if (target.hp >= oldhp) {
    return false;
  }
  if (target.isActor() && !$gameSystem._bmotion[0]) {
    return false;
  }
  if (target.isEnemy() && !$gameSystem._bmotion[1]) {
    return false;
  }
  return true;
};
...
Game_Action.prototype.apply = function (target) {
  ...
  if (this.needSetBmotionDamage(target, oldHP)) {
    target.setBmotionDamageApply(target);
  }
};
...
Game_Battler.prototype.setMotionData = function () {
  ...
  this._bmotion.breathEffect.rangeMax = 0.001;
  ...
  this._bmotion.idle.float.shadow =
    Moghunter.b_motion_shadow == "true" ? true : false;
  this._bmotion.idle.float.range = 1.0;
  ...
  if (Imported.MOG_BattleCameraFrontal) {
    this._bmotion.idle.float.groundH -=
      (Moghunter.b_motion_groundHeight * 50) / 100;
  }
  this._bmotion.idle.swing = {};
  this._bmotion.idle.swing.mode = 0;
  this._bmotion.idle.swing.enable = false;
  this._bmotion.idle.swing.range = 0.3;
  ...
  this._bmotion.damage.mode = Math.min(
    Math.max(Moghunter.b_motion_defaultDamage, 0),
    3
  );
...
Game_Battler.prototype.clearBaction = function () {
  ...
  if (this._batPoses && this._batPoses[2] > 0) {
    this._batPoses[2] = 1;
  }
};
...
Game_Battler.prototype.clearBidleWalk = function () {
  this._bmotion.idle.walk.rangeRealM =
    (this._bmotion.idle.walk.rangeReal * 70) / 100;
...
Game_Battler.prototype.needRefreshBmotionWalk = function (x, y) {
  if (this._bmotion.idle.walk.skip > 0) {
    return false;
  }
  if (this._bmotion.idle.walk.x < this._bmotion.idle.walk.rangeX1) {
    return true;
  }
  if (this._bmotion.idle.walk.x > this._bmotion.idle.walk.rangeX2) {
    return true;
  }
  if (this._bmotion.idle.walk.y < this._bmotion.idle.walk.rangeY1) {
    return true;
  }
  if (this._bmotion.idle.walk.y > this._bmotion.idle.walk.rangeY2) {
    return true;
  }
  if (x < 0) {
    return true;
  }
  if (x > this._bmotion.idle.walk.rangeX3) {
    return true;
  }
  if (y < 0) {
    return true;
  }
  if (y > this._bmotion.idle.walk.rangeY3) {
    return true;
  }
  if (this._bmotion.idle.walk.duration <= 0) {
    return true;
  }
  return false;
};
...
Game_Battler.prototype.setBmotionWalkD = function (value, x, y) {
  ...
  if (value > this._bmotion.idle.walk.rangeRealM) {
    return 1;
  }
  if (value < -this._bmotion.idle.walk.rangeRealM) {
    return 0;
...
Game_Battler.prototype.refreshBmotionWalk = function (x, y) {
  ...
  if (
    !this._bmotion.idle.walk.start &&
    wait < this._bmotion.idle.walk.frequence
  ) {
    this._bmotion.idle.walk.sX = 0;
    this._bmotion.idle.walk.sY = 0;
...
Sprite_Battler.prototype.updateBmotionWalk = function (x, y) {
  if (this._battler._bmotion.idle.walk.wait > 0) {
    this._battler._bmotion.idle.walk.wait--;
    return;
  }
  if (this._battler._bmotion.idle.walk.skip > 0) {
    this._battler._bmotion.idle.walk.skip--;
  }
  ...
  if (this._battler.needRefreshBmotionWalk(x, y)) {
    this._battler.refreshBmotionWalk(x, y);
  }
};
...
Game_Battler.prototype.bmotionX = function () {
  return (
    this._bmotion.x_Offset +
    this._bmotion.action.x +
    this._bmotion.idle.x +
    Math.abs(this._bmotion.damage.x) +
    this._bmotion.idle.walk.x +
    this._bmotion.idle.float.x
  );
};
...
Game_Battler.prototype.bmotionY = function () {
  return (
    this._bmotion.y_Offset +
    this.bmotionFlyY() +
    this._bmotion.action.y +
    this._bmotion.idle.y +
    this._bmotion.damage.y +
    this._bmotion.idle.walk.y +
    this._bmotion.idle.swing.y
  );
};
...
Game_Battler.prototype.bmotionScaleX = function () {
  var n =
    this._bmotion.action.scaleX +
    this._bmotion.idle.scaleX +
    this._bmotion.damage.scaleX;
  if (Imported.MOG_BattleCameraFrontal) {
    n += this.camScaleX();
  }
  return n;
...
Game_Battler.prototype.bmotionScaleY = function () {
  var n =
    this._bmotion.action.scaleY +
    this._bmotion.idle.scaleY +
    this._bmotion.damage.scaleY;
  if (Imported.MOG_BattleCameraFrontal) {
    n += this.camScaleY();
  }
  return n;
...
Game_Battler.prototype.bmotionRotation = function () {
  return (
    this._bmotion.action.rotation +
    this._bmotion.idle.rotation +
    this._bmotion.idle.swing.rotation +
    this._bmotion.damage.rotation
  );
};
...
//==============================
var _mog_bmotion_sprtEnemy_initVisibility =
...
//==============================
const _mog_battlerMotion_spriteEnemy_createStateIconSprite =
...
//==============================
const _mog_battlerMotion_spriteset_battle =
...
EnemyIconsSprites.prototype.isVisible = function () {
  if (!this._enemy_sprite.visible) {
    return false;
  }
  if (!this._enemy_sprite._battler) {
    return false;
  }
  if (this._enemy_sprite._battler.allIcons().length == 0) {
    return false;
  }
  if (this._enemy_sprite._battler.isDead()) {
    return false;
  }
  return true;
};
...
Sprite_Battler.prototype.setBmotionIdle = function () {
  ...
  if (this._battler._bmotion.idle.mode >= 0) {
    this.setupBmotionBreath();
  }
  if (this._battler._bmotion.idle.walk.enable) {
    this._battler.refreshBmotionWalk();
  }
...
Sprite_Battler.prototype.updateIdleBmotion = function () {
  if (this._battler._bmotion.idle.animation[10] > 0) {
    this._battler._bmotion.idle.animation[10]--;
    return;
  }
  if (this.canUpdateWalk()) {
    this.updateBmotionWalk(this.x, this.y);
  }
...
Sprite_Battler.prototype.setupBmotionSwingWave = function () {
  ...
  this._battler._bmotion.idle.swing.animation[2] =
    0.00048 + 0.01 / this.bitmap.height + rds2;
  this._battler._bmotion.idle.swing.animation[3] =
    0.0096 + 0.25 / this.bitmap.height;
...
Sprite_Battler.prototype.updateBmotionSwingWave = function () {
  ...
  if (this._battler._bmotion.idle.swing.animation[0] == 0) {
    var speed =
      this._battler._bmotion.idle.swing.speed +
      Math.abs(this._battler._bmotion.idle.swing.rotation - maxAngle) * 0.015;
    ...
    var speed =
      this._battler._bmotion.idle.swing.speed +
      Math.abs(-this._battler._bmotion.idle.swing.rotation - maxAngle) * 0.01;
    this._battler._bmotion.idle.swing.rotation -= speed;
    if (this._battler._bmotion.idle.swing.rotation <= -maxAngle) {
      this._battler._bmotion.idle.swing.rotation = -maxAngle;
      this._battler._bmotion.idle.swing.animation[0] = 0;
...
Sprite_Battler.prototype.updateBmotionSwing360 = function () {
  this._battler._bmotion.idle.swing.rotation +=
    this._battler._bmotion.idle.swing.speed;
};
...
Sprite_Battler.prototype.canUpdateIdleBMotion = function () {
  if (this.canUpdateActionBMotion()) {
    return false;
  }
...
Sprite_Battler.prototype.canUpdateActionBMotion = function () {
  if (this._battler._bmotion.action.mode < 0) {
    return false;
  }
  return true;
};
...
Sprite_Battler.prototype.setupBMotionIdle1 = function () {
  ...
  this._battler._bmotion.breathEffect.rangeMax = 0.0017;
  ...
  this._battler._bmotion.breathEffect.speed = 0.00006 + Number(sp);
  this._battler._bmotion.breathEffect.scale = 0.0;
};
...
Sprite_Battler.prototype.updateBMotionIdle1 = function () {
  if (this._battler._bmotion.breathEffect.phase == 0) {
    this._battler._bmotion.breathEffect.range -=
      this._battler._bmotion.breathEffect.speed;
    if (
      this._battler._bmotion.breathEffect.range <=
      -this._battler._bmotion.breathEffect.rangeMax
    ) {
      this._battler._bmotion.breathEffect.range =
        -this._battler._bmotion.breathEffect.rangeMax;
  ...
  this._battler._bmotion.breathEffect.scale -=
    this._battler._bmotion.breathEffect.range;
  this._battler._bmotion.idle.scaleY =
    this._battler._bmotion.breathEffect.scale;
};
...
Sprite_Battler.prototype.updateBMotionIdle2 = function () {
  ...
  this._battler._bmotion.breathEffect.scale -=
    this._battler._bmotion.breathEffect.range;
  this._battler._bmotion.idle.scaleX =
    -this._battler._bmotion.breathEffect.scale;
  this._battler._bmotion.idle.scaleY =
    this._battler._bmotion.breathEffect.scale;
};
...
Sprite_Battler.prototype.setupBMotionIdle3 = function () {
  ...
  var rds = Math.randomInt(100);
  var rds2 = rds * 0.0000001;
  this._battler._bmotion.idle.animation[2] =
    0.00001 + 0.01 / this.bitmap.height + rds2;
...
Sprite_Battler.prototype.updateBMotionIdle3 = function () {
  if (this._battler._bmotion.idle.animation[0] == 0) {
    this._battler._bmotion.idle.animation[1] -=
      this._battler._bmotion.idle.animation[2];
    ...
    if (
      this._battler._bmotion.idle.animation[1] <=
      -this._battler._bmotion.idle.animation[3]
    ) {
      this._battler._bmotion.idle.animation[1] =
        -this._battler._bmotion.idle.animation[3];
...
Sprite_Battler.prototype.setupBmotionFloat = function () {
  ...
  var rds = Math.randomInt(100);
  var rds2 = rds * 0.0001;
  this._battler._bmotion.idle.float.animation[2] =
    this._battler._bmotion.idle.float.speed + rds2;
  this._battler._bmotion.idle.float.animation[3] =
    this._battler._bmotion.idle.float.range;
...
Sprite_Battler.prototype.updateBMotionFloat = function () {
  ...
  if (this._battler._bmotion.idle.float.animation[0] == 0) {
    this._battler._bmotion.idle.float.animation[1] -=
      this._battler._bmotion.idle.float.animation[2];
    this._battler._bmotion.idle.float.animation[4] = this.setBmotionFloatMode();
    this._battler._bmotion.idle.float.y +=
      this._battler._bmotion.idle.float.animation[1];
    this._battler._bmotion.idle.float.x +=
      this._battler._bmotion.idle.float.animation[4];
    this._d[0] = 0;
    if (
      this._battler._bmotion.idle.float.animation[1] <=
      -this._battler._bmotion.idle.float.animation[3]
...
Sprite_Battler.prototype.updateActionBmotion = function () {
  if (this._battler._bmotion.action.initial) {
    this.setupInitialBmotionAction();
  }
...
Sprite_Battler.prototype.isBJumpAction = function () {
  if (this._battler._bmotion.action.mode == 3) {
    return true;
  }
  if (this._battler._bmotion.action.mode == 4) {
    return true;
  }
  if (this._battler._bmotion.action.mode == 10) {
    return true;
  }
  return false;
};
...
Sprite_Battler.prototype.setupBMotionActionZoom1 = function () {
  this._battler._bmotion.action.animation[1] = 0.005 + 1.0 / this.bitmap.height;
};
...
Sprite_Battler.prototype.updateBMotionActionZoom1 = function () {
  ...
  if (this._battler._bmotion.action.animation[0] < 15) {
    this._battler._bmotion.action.scaleX +=
...
Sprite_Battler.prototype.updateBMotionSwingRight = function () {
  ...
  if (this._battler._bmotion.action.animation[0] < 10) {
    ...
    if (Imported.MOG_BattleCameraFrontal) {
      this._battler._bmotion.action.scaleX =
        -2.0 - this._battler.camScaleX() * 2.0;
    } else {
      this._battler._bmotion.action.scaleX = -2.0;
    }
  } else if (this._battler._bmotion.action.animation[0] < 30) {
    this._battler._bmotion.action.x -= speed;
...
Sprite_Battler.prototype.updateBmotionJump = function () {
  this._battler._bmotion.action.animation[1] += 0.3;
  this._battler._bmotion.action.y2 +=
    this._battler._bmotion.action.animation[1];
...
Sprite_Battler.prototype.updateBmotionJumpRoll = function () {
  ...
  if (this._battler._bmotion.action.animation[0] > 5) {
    ...
    if (Imported.MOG_BattleCameraFrontal) {
      var s = -2.0 - this._battler.camScaleX() * 2.0;
      this._battler._bmotion.action.scaleX =
        this._battler._bmotion.action.scaleX == 0 ? s : 0;
    } else {
      this._battler._bmotion.action.scaleX =
        this._battler._bmotion.action.scaleX == 0 ? -2.0 : 0;
    }
  }
};
...
Sprite_Battler.prototype.updateBmotionJumpSlime = function () {
  if (this._battler._bmotion.action.animation[0] == 0) {
    ...
    if (this._battler._bmotion.action.scaleX >= 2.5) {
    ...
    if (this._battler._bmotion.action.scaleX >= 0.0) {
      this._battler._bmotion.action.scaleX -= 0.14;
      if (this._battler._bmotion.action.scaleX < 0.0) {
        this._battler._bmotion.action.scaleX = 0;
      }
    }
    if (this._battler._bmotion.action.scaleY < 0.0) {
...
Sprite_Battler.prototype.updateBmotionSideRightAttackCharge = function () {
  if (this._battler._bmotion.action.animation[0] == 0) {
    ...
    this._battler._bmotion.action.y +=
      this._battler._bmotion.action.animation[1];
    var speed = this._battler.isActor() ? -3 : 3;
    this._battler._bmotion.action.x -= speed;
    if (this._battler._bmotion.action.animation[1] >= 8) {
    ...
    this._battler._bmotion.action.animation[0] = 2;
    this._battler._bmotion.action.animation[3] =
      -this._battler._bmotion.action.x;
...
Sprite_Battler.prototype.updateBmotionShakeAttack = function () {
  this._battler._bmotion.action.animation[0]++;
  this._battler._bmotion.action.animation[1]++;
  if (this._battler._bmotion.action.animation[1] < 2) {
    return;
  }
...
Sprite_Battler.prototype.updateBmotionBase = function () {
  if (this.canUpdateIdleBMotion()) {
    this.updateIdleBmotion();
  }
  if (this.canUpdateActionBMotion()) {
    this.updateActionBmotion();
  }
  if (this.canUpdateDamageBmotion()) {
    this.updateDamageBmotion();
  }
};
...
Sprite_Battler.prototype.updateBmotionDamageShake = function () {
  ...
  if (this._battler._bmotion.damage.animation[0] < 1) {
    return;
  }
...
Sprite_Battler.prototype.updateBmotionDamageZoom = function () {
  ...
  if (this._battler._bmotion.damage.animation[0] == 0) {
    ...
    this._battler._bmotion.damage.scaleX += speed;
    this._battler._bmotion.damage.scaleY -= speed;
    if (this._battler._bmotion.damage.scaleX >= 0.0) {
      this._battler.clearBdamage();
      this._movementDuration = 0;
    }
  }
...
Sprite_Enemy.prototype.initMembers = function () {
  ...
  this._skipBlink =
    String(Moghunter.b_motion_disable_blink_damage) == "true" ? true : false;
};
...
Sprite_Battler.prototype.canUpdateBattlerMotion = function () {
  ...
  if (Imported.MOG_EmergeMotion) {
    if (this._battler._emergeMotion.enabled) {
      return false;
    }
  }
  return true;
...
Sprite_Battler.prototype.updateBattlerMotion = function () {
  if (this.needGetDataBmotion()) {
    this.setInitBmotionData();
  }
  if (this.canUpdateBmotion()) {
    this.updateBmotionBase();
  }
  if (this._battler) {
    this.updatepBmotionRealData();
  }
};
...
Sprite_Battler.prototype.update = function () {
  ...
  if (this.canUpdateBattlerMotion()) {
    this.updateBattlerMotion();
  }
};
...
Sprite_Battler.prototype.updatepBmotionRealData = function () {
  ...
  if (!this._battler.isDead()) {
    this.scale.x = 1.0 + this._battler.bmotionScaleX();
    this.scale.y = 1.0 + this._battler.bmotionScaleY();
...
Sprite_Battler.prototype.needWaitUpdateMove = function () {
  ...
  if (this._battler._bmotion.action.wait) {
    return true;
  }
  if (this._battler._bmotion.idle.wait) {
    return true;
  }
  if (this._battler._bmotion.damage.wait) {
    return true;
  }
  return false;
};
...
// * update Idle Bmotion
//==============================
var _mog_bmotion_sprtBattler_updateIdleBmotion =
  Sprite_Battler.prototype.updateIdleBmotion;
Sprite_Battler.prototype.updateIdleBmotion = function () {
  _mog_bmotion_sprtBattler_updateIdleBmotion.call(this);
  if (this.needCreateBmotionShadow()) {
...
Sprite_Battler.prototype.createBmotionShadow = function () {
  ...
  this._bmotionShadow.zoomEffect =
    Moghunter.b_motion_shadowZoom == "true" ? true : false;
  this._bmotionShadow.scale.y = 0.2;
...
Sprite_Battler.prototype.updateBmotionShadow = function () {
  ...
  this._bmotionShadow.y =
    mz + this._bmotionShadow.org[1] - this._battler.bmotionFlyY() + nz + rz;
...
// * update Bmotion Base
//==============================
var _mog_bmotion_sprtBattler_updateBmotionBase =
  Sprite_Battler.prototype.updateBmotionBase;
Sprite_Battler.prototype.updateBmotionBase = function () {
  _mog_bmotion_sprtBattler_updateBmotionBase.call(this);
  if (this._bmotionShadow) {
    this.updateBmotionShadow();
  }
};
