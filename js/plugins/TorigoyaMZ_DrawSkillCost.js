...
(function () {
  ...
  function readParameter() {
    ...
    return {
      version: "1.1.1",
      labelFontSize: pickNumberValueFromParameter(
        parameter,
        "labelFontSize",
        14
      ),
      valueFontSize: pickNumberValueFromParameter(
        parameter,
        "valueFontSize",
        20
      ),
  ...
  (() => {
    ...
    Window_SkillList.prototype.drawSkillCost = function (skill, x, y, width) {
      ...
      const maxWidth =
        (width -
          Torigoya.DrawSkillCost.parameter.gapSize * (items.length - 1)) /
        items.length;
      let baseX = x + width;
      items.forEach((obj) => {
        ...
        const w = Math.max(
          Math.min(
            this.contents.measureTextWidth(obj.value),
            maxWidth - labelWidth
          ),
          1
        );
        ...
        if (Torigoya.DrawSkillCost.parameter.labelFontSize) {
          this.contents.fontSize =
            Torigoya.DrawSkillCost.parameter.labelFontSize;
          baseX -= labelWidth;
    ...
    Window_SkillList.prototype.torigoyaDrawSkillCost_costItems = function (
      skill
    ) {
      const hpCost =
        typeof this._actor.skillHpCost === "function"
          ? this._actor.skillHpCost(skill)
          : 0;
      ...
      return [
        ...
        mpCost
          ...
          : false,
        hpCost
          ? { value: hpCost, label: TextManager.hpA, color: getHpCostColor() }
          : false,
      ].filter(Boolean);
    };
  })();
