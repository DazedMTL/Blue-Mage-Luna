...
(function () {
  ...
  PluginManager.registerCommand(pluginName, "createFilter", function (args) {
    ...
    const posRefTargetId = parseFilterPosRefId(
      args.positionReferenceTargetId,
      this
    );
    $gameMap.createFilter(
      filterId,
      filterType,
      filterTarget,
      filterTargetIds,
  ...
  const parseTargetIdArray = function (idTextArrayText, interpreter) {
    ...
    const idTextArray = JSON.parse(idTextArrayText),
      targetIdArray = [];
  ...
  const parseTargetId = function (idText, interpreter) {
    ...
    let sub = BattleManager._subject,
      index = 0;
    if (sub) {
  ...
  Filter_Controller.targetType["Water"] = "Water";
  ...
  Filter_Controller.targetType["ExcludeSpecificPicture"] =
    "ExcludeSpecificPicture";
  ...
  Filter_Controller.prototype.initialize = function (
    filterName,
    id,
    targetType,
    targetIds,
  ...
  Filter_Controller.prototype.initBasic = function (
    id,
    targetType,
    targetIds,
    posRefCharId,
  ...
  Filter_Controller.prototype.createDisplacement = function () {
    if (!SceneManager._scene._TKMDisplacementMap) {
      SceneManager._scene._TKMDisplacementMap = new Sprite(
        ImageManager.loadPicture(TKMDisplacementMap)
      );
      SceneManager._scene.addChild(SceneManager._scene._TKMDisplacementMap);
    }
    const f = new PIXI.filters.DisplacementFilter(
      SceneManager._scene._TKMDisplacementMap
    );
  ...
  Filter_Controller.prototype.updateMove = function () {
    ...
    if (this._duration > 0) {
      let d = this._duration,
        cp = this.currentParams,
        tp = this.targetParams;
  ...
  Filter_Controller.prototype.checkErase = function () {
    if (typeof this._posRefCharId === "number") {
      if (this._posRefCharMapId >= 0 && !this.isOnCurrentMap()) {
        ...
        if (this._posRefCharId >= 0) this.erase();
  ...
  _updateFilterHandler["bulgepinch"] = function (filter, cp) {
    ...
    filter.center = [
      (loc[0] + cp[0]) / Graphics.width,
      (loc[1] + cp[1]) / Graphics.height,
    ];
    filter.radius = cp[2];
  ...
  _updateFilterHandler["rgbsplit"] = function (filter, cp) {
    let r = cp[0],
      sita = cp[1];
    filter.red = [
      r * Math.sin((Math.PI / 180) * sita),
      r * Math.cos((Math.PI / 180) * sita),
    ];
    sita += 120;
    filter.green = [
      r * Math.sin((Math.PI / 180) * sita),
  ...
  _updateFilterHandler["displacement"] = function (filter, cp) {
    ...
    //////////////////////////////////////////////////////////////////
    //画面補正	スクロールに影響されないようにします。
    filter.maskSprite.x -= $gameMap.charSpeedX;
    filter.maskSprite.y -= $gameMap.charSpeedY;
    //////////////////////////////////////////////////////////////////
  ...
  Filter_Controller.prototype.getCharLoc = function () {
    ...
    if (!!this._posRefCharMapId) {
      // search event screen Loc
      ...
      // search Battler screen X
      if (!$gameParty.inBattle()) return [0, 0];
      let char;
      if (this._posRefCharId < 0)
        char = BattleManager._spriteset._enemySprites[-this._posRefCharId - 1];
      else
        char = BattleManager._spriteset._actorSprites[this._posRefCharId - 1];
  ...
  Game_Map.prototype.createFilter = function (
    id,
    filter,
    targetType,
    targetIds,
    ...
    let mapId =
      typeof posRefTargetId === "number" && !$gameParty.inBattle()
        ? this.mapId()
        : null;
    const fc = new Filter_Controller(
      filter,
      id,
      targetType,
      targetIds,
  ...
  const _fullScreenTargetTypes = [
    ...
    Filter_Controller.targetType.Water,
  ];
  ...
  Scene_Base.prototype.getTKMFilterObj = function (targetType, targetIds) {
    ...
    if (typeof typeTargetGetter !== "function") {
      console.warn(
        "指定された適用対象タイプが存在していない。大・小文字 / 書き間違いにご注意ください"
      );
      console.warn(
        "The specified target type does not exist. Is there any upper-lower cases mismatch or typos?"
      );
      return [];
    }
  ...
  // defenition of each TargetType's target
  {
    ...
    targetGetter[Type.TileAndWater] = function (targetIds) {
      const targets = [];
      if (this._spriteset && this._spriteset._tilemap) {
        targets.push(this._spriteset._tilemap._lowerLayer);
        targets.push(this._spriteset._tilemap._upperLayer);
        targets.push(this._spriteset._parallax);
        targets.push(this._spriteset._water);
      }
      return targets;
    };
    ...
    targetGetter[Type.Water] = function (targetIds) {
      const targets = [];
      if (this._spriteset) {
        targets.push(this._spriteset._water);
      }
    ...
    targetGetter[Type.SepcificChar] = function (targetIds) {
      ...
      targetIds.forEach((targetId) => {
        targets.push(
          _getCharSpriteById(charSprites, targetId, defaultArrLength)
        );
      });
      return targets;
    };
    ...
    targetGetter[Type.ExcludeSepcificChar] = function (targetIds) {
      ...
      targetIds.forEach((targetId) => {
        const charSprite = _getCharSpriteById(
          charSprites,
          targetId,
          defaultArrLength
        );
  ...
  const _defaultCharSpriteArrayLength = function () {
    return (
      ...
      $gameMap.vehicles().length +
      $gamePlayer.followers()._data.length +
      1
    ); //player
  };
  ...
  const _getCharSpriteById = function (charSprites, id, length) {
    ...
    if (id < 0) {
      ...
      if (
        maybeTarget &&
        maybeTarget._character &&
        maybeTarget._character.eventId() === id
      ) {
        return maybeTarget;
      } else {
        let target = charSprites.find(
          (sprite_char) =>
            sprite_char._character && sprite_char._character._eventId === id
        );
  ...
  Scene_Map.prototype.updateTKMfilters = function () {
    ...
    this._TKMFilters = this._TKMFilters.filter((filter) => {
      ...
      if (erased) {
        this.removeTKMFilterFromTarget(filter);
      }
      return !erased;
    }, this);
    // 存在しない -> 作成
  ...
  var ze = a,
    ...
    et = (function (e) {
      function t(t) {
        ...
        Object.assign(
          this,
          { strength: 0.1, center: [0, 0], innerRadius: 0, radius: -1 },
          t
        );
      }
      e && (t.__proto__ = e),
...
var __filters = (function (n, e, t) {
  ...
  var i =
    ...
    a = (function (n) {
      function e(e) {
        n.call(this, i, r.replace("${perlin}", o)),
          (this.uniforms.dimensions = new Float32Array(2)),
          "number" == typeof e &&
            (console.warn(
              "GodrayFilter now uses options instead of (angle, gain, lacunarity, time, strength)"
            ),
            (e = { angle: e }),
            void 0 !== arguments[1] && (e.gain = arguments[1]),
            void 0 !== arguments[2] && (e.lacunarity = arguments[2]),
            void 0 !== arguments[3] && (e.time = arguments[3]),
            void 0 !== arguments[4] && (e.strength = arguments[4])),
          (e = Object.assign(
            {
              angle: 30,
              gain: 0.5,
      ...
      return (
        (e.prototype.apply = function (n, e, t, i) {
          var o = e.filterFrame,
            r = o.width,
            a = o.height;
          (this.uniforms.light = this.parallel
            ...
            : this.center),
            (this.uniforms.parallel = this.parallel),
            (this.uniforms.dimensions[0] = r),
            (this.uniforms.dimensions[1] = a),
            (this.uniforms.aspect = a / r),
            (this.uniforms.time = this.time),
            n.applyFilter(this, e, t, i);
        }),
        (a.angle.get = function () {
          return this._angle;
        }),
        (a.angle.set = function (n) {
          this._angle = n;
          var e = n * t.DEG_TO_RAD;
        ...
        (a.gain.set = function (n) {
          this.uniforms.gain = n;
        }),
        (a.lacunarity.get = function () {
          return this.uniforms.lacunarity;
        }),
        (a.lacunarity.set = function (n) {
          this.uniforms.lacunarity = n;
        }),
        (a.strength.get = function () {
          return this.uniforms.strength;
        }),
        (a.strength.set = function (n) {
          this.uniforms.strength = n;
        }),
        Object.defineProperties(e.prototype, a),
        e
      );
    })(e.Filter);
  return (n.GodrayFilter = a), n;
})({}, PIXI, PIXI);
Object.assign(PIXI.filters, __filters);
