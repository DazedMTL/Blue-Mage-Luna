...
(() => {
  ...
  function nuun_GausePlugins() {
    return (
      Imported.NUUN_ButlerHPGauge ||
      Imported.NUUN_EnemyMPGauge ||
      Imported.NUUN_EnemyTPGauge ||
      Imported.NUUN_ButlerName ||
      Imported.NUUN_EnemyTpbGauge ||
      Imported.NUUN_EnemyStateIconEX
    );
  }
  ...
  NuunManager.numPercentage = function (num, digits, mode) {
    return (
      (mode
        ? Math.round(num * Math.pow(10, digits + 2))
        : Math.floor(num * Math.pow(10, digits + 2))) / Math.pow(10, digits + 2)
    );
  };
  ...
  const _Window_Selectable_drawItemBackground =
    Window_Selectable.prototype.drawItemBackground;
  ...
  Spriteset_Battle.prototype.updateButlerGauge = function () {
    if (this._butlerGaugeBase) {
      for (const sprite of this._butlerGaugeBase.children) {
        const spriteData = this._enemySprites.some(
          (enemy) => enemy.spriteId === sprite.enemySpriteId
        );
        if (!spriteData) {
  ...
  Game_Action.prototype.getAttackElementsList = function () {
    return Imported.NUUN_MultiElement
      ? this.getAttackElements()
