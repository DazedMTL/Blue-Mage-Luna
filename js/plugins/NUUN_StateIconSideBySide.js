...
(() => {
  ...
  const ActorStateIconShowVal = Number(
    parameters["ActorStateIconShowVal"] || 5
  );
  const ActorStateIconRows = Number(parameters["ActorStateIconRows"] || 1);
  const EnemyStateIconShowVal = Number(
    parameters["EnemyStateIconShowVal"] || 1
  );
  const EnemyStateIconRows = Number(parameters["EnemyStateIconRows"] || 1);
  const ActorStateIconAlign =
    eval(parameters["ActorStateIconAlign"]) || "right";
  const EnemyStateIconAlign =
    eval(parameters["EnemyStateIconAlign"]) || "center";
  const ActorStateIconPosition =
    eval(parameters["ActorStateIconPosition"]) || "auto";
  const ActorStateIconVisible = eval(
    parameters["ActorStateIconVisible"] || "true"
  );
  const EnemyStateIconVisible = eval(
    parameters["EnemyStateIconVisible"] || "true"
  );
  ...
  Sprite_StateIcon.prototype.bitmapWidth = function () {
    return Math.min(
      ImageManager.iconWidth * this.getStateIconShowVal(),
      StateIconWidth
    );
  };
  ...
  Sprite_StateIcon.prototype.createSprite = function () {
    for (
      let i = 0;
      i < this.getMaxStateIconShowVal() * this.getMaxStateIconRows();
      i++
    ) {
  ...
  Sprite_StateIcon.prototype.stateIconDisplayAlign = function (
    iconlength,
    align
  ) {
    if (align === "center") {
  ...
  Sprite_StateIcon.prototype.createStateIcons = function (icons, turns) {
    ...
    let displayTurn = [];
    if (icons.length > 0) {
      displayIcons = icons.slice(
        this._animationIndex,
        this._animationIndex +
          this.getStateIconShowVal() * this.getStateIconRows()
      );
      displayTurn = turns.slice(
        this._animationIndex,
        this._animationIndex +
          this.getStateIconShowVal() * this.getStateIconRows()
      );
  ...
  Sprite_StateIcon.prototype.updateFrame = function () {
    ...
    this._iconSprite.forEach((sprite, r) => {
      sprite.x =
        Math.floor(r % this.getStateIconShowVal()) * this.iconX(showLength) +
        this.stateIconDisplay(showLength);
      sprite.y =
        Math.floor(r / this.getStateIconShowVal()) * ImageManager.iconHeight;
      if (sprite.visible) {
        this.setFrameIcon(sprite);
        this.setTurn(sprite);
      }
  ...
  Sprite_StateIcon.prototype.iconX = function (iconsLength) {
    if (
      StateIconWidth > 0 &&
      ImageManager.iconWidth * iconsLength > StateIconWidth
    ) {
  ...
  Sprite_StateIcon.prototype.getStateIconShowVal = function () {
    return this._battler && this._battler.isActor()
      ? ActorStateIconShowVal
      : EnemyStateIconShowVal;
  };
  ...
  Game_BattlerBase.prototype.nuun_stateTurns = function () {
    return this.states().reduce((r, state) => {
      if (state.iconIndex > 0) {
        const turn = [
          this.nuun_isNonRemoval(state) ? 0 : this.nuun_getStateTurn(state.id),
        ];
  ...
  Game_BattlerBase.prototype.nuun_getStateTurn = function (id) {
    return (
      (Imported.NUUN_StateTurnCount && TurnMode === "elapsed"
        ? this.getStateNowTurn(id)
        : this._stateTurns[id]) + TurnCorrection
    );
  };
  ...
  Game_BattlerBase.prototype.nuun_getBuffTurn = function (id) {
    return (
      (Imported.NUUN_StateTurnCount && TurnMode === "elapsed"
        ? this.getBuffNowTurn(id)
        : this._buffTurns[id]) + TurnCorrection
    );
  };
})();
