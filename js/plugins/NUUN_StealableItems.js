...
(() => {
  ...
  const NotStealName = String(
    parameters["NotStealName"] || "%2から何も盗めなかった！"
  );
  const NonStealName = String(
    parameters["NonStealName"] || "%2は何も持っていない！"
  );
  const NonStealGoldName = String(
    parameters["NonStealGoldName"] || "%2は何も持っていない！"
  );
  const GetStealName = String(
    parameters["GetStealName"] || "%2から%3を盗んだ！"
  );
  const StolenName = String(
    parameters["StolenName"] || "%2から%3を盗み取った！"
  );
  const stolenItems =
    (NUUN_Base_Ver >= 113
      ? DataManager.nuun_structureData(parameters["stolenItems"])
      : null) || [];
  ...
  function getStolenItemList(target, rate) {
    ...
    if (target.getStolenRate(rate) && stolenItems) {
      let stolenItemList = stolenItems.reduce(function (r, item) {
        if (
          $gameParty._items[item.stolenItemId] > 0 &&
          $gameSystem.stolenSwitch(item)
        ) {
  ...
  Game_System.prototype.stolenSwitch = function (item) {
    return item.stolenSwitch > 0
      ? $gameSwitches.value(item.stolenSwitch)
      : true;
  };

  const _Game_Action_applyItemUserEffect =
  ...
  Game_Action.prototype.getSteal = function (target) {
    ...
    if (target.result().isHit()) {
      if (target.isEnemy()) {
        ...
        if (
          this.item().meta.goldStealSkill ||
          this.item().meta.goldStealSkillRate
        ) {
  ...
  Game_Action.prototype.lostStolenGold = function (target, rate, stolenGold) {
    ...
    if (target.getStolenRate(rate)) {
      if (stolenGold[2] === 1) {
        gold = Math.floor(
          $gameParty._gold * Number(Math.min(stolenGold[1], 100) / 100)
        );
      } else {
  ...
  Game_BattlerBase.prototype.stealPercentBoost = function () {
    return this.traitObjects().reduce((r, trait) => {
      return (
        r *
        (trait.meta.steal_sr_Percent
          ? Number(trait.meta.steal_sr_Percent) / 100
          : 1.0)
      );
    }, 1.0);
  };
  ...
  Game_BattlerBase.prototype.stealItemRate = function () {
    return this.traitObjects().reduce((r, trait) => {
      return (
        r *
        (trait.meta.stealResist ? Number(trait.meta.stealResist) / 100 : 1.0)
      );
    }, 1.0);
  };
  ...
  Game_Enemy.prototype.stealConditions = function (di) {
    if (Imported.NUUN_ConditionsBase && di.cond) {
      ...
      return action.subject.getTriggerConditions(
        this.enemy(),
        this,
        condTag,
        "Target" + condTag,
  ...
  Game_Enemy.prototype.makeStealItems = function (rate) {
    ...
    for (const di of this._stealItems) {
      if (
        di.kind > 0 &&
        di.kind < 4 &&
        this.stealConditions(di) &&
        randomRate(di.id, stealId) &&
        this.getStealRate(rate, di)
      ) {
  ...
  Game_Enemy.prototype.getStealRate = function (rate, di) {
    return (
      ...
      rate * ((di.denominator * this.stealItemRate()) / 100)
    );
  };
        ...
        switch (match[1]) {
          case "I":
            this._stealItems.push({
              dataId: parseInt(data[0]),
              denominator: parseInt(data[1]),
              kind: 1,
              cond: data[2],
              mode: parseInt(data[3]),
            });
            break;
          case "W":
            this._stealItems.push({
  ...
  Window_BattleLog.prototype.displaySteal = function (subject, target) {
    StealMessage.forEach((message) => {
      this.push(
        "addText",
        message.text.format(subject.name(), target.name(), message.item)
      );
    });
    StealMessage = [];
  };
