...
(function () {
  ...
  const pActingNoStateAnimation = toBoolean(
    parameters["actingNoStateAnimation"],
    false
  );
  ...
  function showMapAnimation(mapAnimationParams) {
    ...
    // trigger:4（並列処理の場合）
    if (event && event._trigger == 4 && isOutOfRange(event, targetRangeGrid)) {
      return;
    }
    ...
    for (const subject of subjects) {
      ...
      const mapAnimation = makeMapAnimation(
        this,
        subject,
        wait,
        noScroll,
      ...
      if (!mapAnimation.noWait) {
        ...
        this._lastDynamicTargetId = this.getDynamicTargetId(
  ...
  PluginManager.registerCommand(
    ...
    function (args) {
      ...
      if (subjectCondition) {
        subject = BattleManager.allBattleMembers().find((a) =>
          eval(subjectCondition)
        );
      }
      // 対象を取得
      let targets = undefined;
      if (targetCondition) {
        targets = BattleManager.allBattleMembers().filter((a) =>
          eval(targetCondition)
        );
      }
   ...
   */
  Game_Interpreter.prototype.startDynamicAnimation = function (
    targets,
    action,
    mapAnimation
  ) {
  ...
  function showBattleAnimation(
    ...
    const mapAnimation = makeMapAnimationBattle(
      this,
      subject,
      wait,
      action,
  ...
  function isParallel(interpreter) {
    ...
    return commonEvents.some(
      (common) => common._interpreter == interpreter && common.isActive()
    );
  }
  ...
  function makeMapAnimationBattle(interpreter, subject, wait, action, targets) {
    ...
    if (!mapAnimation.noWait) {
      ...
      // タイミングを測るためアニメーションの最終対象を保持しておく
      interpreter._lastDynamicTargetId = interpreter.getDynamicTargetId(
        targets[targets.length - 1]
      );
      // モーション判定用の行動主体
   ...
   */
  Game_CharacterBase.prototype.setDynamicAnimation = function (
    dynamicAnimation
  ) {
    // 処理をSpriteで実行
   ...
   * ※元々ＭＶのみに存在する処理
   */
  const _Sprite_Character_updateAnimation =
    Sprite_Character.prototype.updateAnimation;
  ...
  Sprite_Character.prototype.isAnimationRequestWait = function () {
    ...
    if (this.isAnimationRequested()) {
      ...
      const noWait = this.animations().every(
        (animation) =>
   ...
   * 【独自】動的アニメーションの呼び出し
   */
  Sprite_Character.prototype.startDynamicAnimation = function (
    dynamicAnimation
  ) {
    // ダメージ処理
    ...
    if (Utils.RPGMAKER_NAME == "MV") {
      ...
      dynamicAnimation.targetsSprite = dynamicAnimation.targetsSprite.map(
        (sprite) => getSprite(sprite._character)
      );
      // 有効なデータに限定
      dynamicAnimation.targetsSprite = dynamicAnimation.targetsSprite.filter(
        (sprite) => sprite
      );

      const spriteset = getSpriteset();
      spriteset.createDynamicAnimation(
        [this._character],
        animation,
        dynamicAnimation
      );
  ...
  Game_Interpreter.prototype.isDynamicAnimationPlaying = function () {
    ...
    if (lastDynamicTarget) {
      ...
      if (sprite) {
        ...
        if (sprite._animationSprites) {
          const onDynamicAnimation = sprite._animationSprites.some(function (
            animationSprite
          ) {
            return (
              animationSprite.dynamicAnimation &&
    ...
    if (spriteset._animationSprites) {
      const animationSprite = spriteset._animationSprites.find(function (
        animationSprite
      ) {
        return (
          animationSprite.dynamicAnimation &&
      ...
      if (animationSprite) {
        ...
        if (
          dynamicAnimation &&
          !dynamicAnimation.mv &&
          dynamicAnimation.baseAnimation.wait == "auto"
        ) {
          return true;
        }
      } else {
   ...
   * ●場所移動
   */
  const _Scene_Map_updateTransferPlayer =
    ...
    if ($gameTemp.animationSprites) {
      for (const oldSprite of $gameTemp.animationSprites) {
        ...
        if (Utils.RPGMAKER_NAME == "MV") {
          ...
          // 保存時とは別のオブジェクトになっているのでＩＤから再取得
          const targetSprites = oldSprite._targets.map((sprite) =>
            this.getNewSprite(sprite)
          );
          // 有効な要素が取得できなければ処理しない。
          // ※ないはずだが念のため。
          ...
          sprite.targetObjects = targetSprites.map(
            (sprite) => sprite._character
          );
          ...
          if (oldSprite.dynamicAnimation) {
            ...
            // スプライトを現在のマップのものへ更新
            dynamicAnimation.referenceSubject = this.getNewSprite(
              dynamicAnimation.referenceSubject
            );
            dynamicAnimation.referenceTarget = this.getNewSprite(
              dynamicAnimation.referenceTarget
            );
            dynamicAnimation.targetsSprite = dynamicAnimation.targetsSprite.map(
              (sprite) => this.getNewSprite(sprite)
            );
            sprite.dynamicAnimation = dynamicAnimation;
          }
  ...
  Spriteset_Map.prototype.getNewSprite = function (oldSprite) {
    ...
    for (const sprite of this._characterSprites) {
      if (sprite._character == character) {
        ...
        character._eventId &&
        sprite._character._eventId === character._eventId
      ) {
        return sprite;
      }
    }
  ...
  Game_Event.prototype.update = function () {
    ...
    if (this._dynamicSkill) {
      ...
      const mapAnimation = makeMapAnimationEvent(
        this,
        this._dynamicSkill,
        action
      );
   ...
   * ※NRP_DynamicReadTxtとの連携用にメソッド分割
   */
  Game_Event.prototype.showDynamicAnimation = function (
    targets,
    action,
    mapAnimation
  ) {
  ...
  Sprite_Battler.prototype.update = function () {
    ...
    if (battler) {
      ...
      for (const skillId of battler.dynamicSkills) {
        ...
        const action = makeAction(skillId);
        const mapAnimation = makeMapAnimationStateAndBattler(
          battler,
          skillId,
          action
        );
   ...
   * ※NRP_DynamicReadTxtとの連携用にメソッド分割
   */
  Game_Battler.prototype.showDynamicAnimation = function (
    targets,
    action,
    mapAnimation
  ) {
   ...
   * 【独自】動的アニメーションの呼び出し
   */
  const _Sprite_Battler_startDynamicAnimation =
    Sprite_Battler.prototype.startDynamicAnimation;
  Sprite_Battler.prototype.startDynamicAnimation = function (
    ...
    if (!dynamicAnimation.noWait) {
      ...
      if (dynamicAnimation.isDynamicAuto) {
        ...
        // より長いほうを採用
        battler.dynamicDurations[skillId] = Math.max(
          waitDuration,
          dynamicDuration
        );
      }
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    if (
      lowerCommand === "nrp.animation.skill" ||
      lowerCommand === "nrp.animation.item"
    ) {
      ...
      lowerCommand === "nrp.animation.startpoint" ||
      lowerCommand === "nrp.animation.subject"
    ) {
