...
(function () {
  ...
  BaseAnimation.prototype.makeRepeatAnimation = function (
    ...
    if (position === 3) {
      ...
      for (const target of distinctTargets) {
        ...
        const dynamicAnimation = this.createDynamicAnimation(
          target,
          delay,
          dynamicAnimationList
        );
  ...
  BaseAnimation.prototype.createDynamicAnimation = function (
    ...
    if (this.position == 3) {
      ...
      dynamicAnimation.targetsSprite = this.distinctTargets.map((battler) =>
  ...
  Spriteset_Base.prototype.isDynamicAnimationPlaying = function () {
    ...
    if (this._requestDynamicAnimation) {
      ...
      var isAnimationRequestWait = this.characterSprites().some((sprite) =>
        sprite.isAnimationRequestWait()
      );
      ...
      var isAnimationRequested = this.characterSprites().some((sprite) =>
        sprite.isAnimationRequested()
      );
   ...
   * ※実際にはアクション実行の終了判定に使う。
   */
  const _Spriteset_Base_isAnimationPlaying =
    Spriteset_Base.prototype.isAnimationPlaying;
  Spriteset_Base.prototype.isAnimationPlaying = function () {
    // 全てのアニメーションがnoWaitならば待たない。
    if (
      this._animationSprites.length > 0 &&
      this._animationSprites.every(
        (sprite) =>
   ...
   */
  Spriteset_Base.prototype.createDynamicAnimation = function (
    targets,
    animation,
    dynamicAnimation
  ) {
    ...
    if (this.isAnimationForEach(animation)) {
      for (const target of targets) {
        this.createDynamicAnimationSprite(
          [target],
          animation,
          mirror,
          delay,
   ...
   * ●アニメーションの削除処理
   */
  const _Spriteset_Base_removeAnimation =
    Spriteset_Base.prototype.removeAnimation;
   ...
   * ●効果音制御
   */
  const _Sprite_Animation_processSoundTimings =
  ...
  Sprite_Animation.prototype.processSoundTimings = function () {
    ...
    if (this.dynamicAnimation) {
      if (
        this.dynamicAnimation.isLimitSound ||
        this.dynamicAnimation.isLimitEffect
      ) {
        return;
      }
    }
   ...
   * ●フラッシュ制御
   */
  const _Sprite_Animation_processFlashTimings =
  ...
  Sprite_Animation.prototype.processFlashTimings = function () {
    ...
    if (this.dynamicAnimation) {
      if (
        this.dynamicAnimation.isLimitFlash ||
        this.dynamicAnimation.isLimitEffect
      ) {
        return;
      }
  ...
  function getBattlerSprite(battler) {
    ...
    // 一致があれば返す
    return sprites.find(
      (s) => s._battler == battler || s._character == battler
    );
  }
  ...
  if (pFvActorHomeX && pFvActorHomeY) {
    ...
    const _Spriteset_Battle_createActors =
