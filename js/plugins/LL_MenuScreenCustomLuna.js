//=============================================================================
// RPGツクールMZ - LL_MenuScreenCustomLuna.js v1.4.3
//-----------------------------------------------------------------------------
// ルルの教会 (Lulu's Church)
 ...
 *
 * @help LL_MenuScreenCustomLuna.js
 *
 ...
 * @desc ※この項目は使用しません
 *
 * @param commandWindowPosition
 * @text メニュー表示位置
 * @desc メニューコマンドの表示位置です。
 * @default right
 * @type select
 * @option 右側
 * @value right
 ...
 * @option メニュー下に別ウィンドウで表示　※上部、下部配置時無効
...
(() => {
  "use strict";
  const pluginName = "LL_MenuScreenCustomLuna";

  const parameters = PluginManager.parameters(pluginName);
  const commandWindowPosition = String(
    parameters["commandWindowPosition"] || "right"
  );
  ...
  const currencyWindowPosition = String(
    parameters["currencyWindowPosition"] || "helpWindowRightBottom"
  );
  // 表示位置の設定
  ...
  const rightBottomBlockLabel = String(
    parameters["rightBottomBlockLabel"] || ""
  );
  const rightBottomBlockValue = String(
    parameters["rightBottomBlockValue"] || ""
  );
  const rightBottomBlockAlign = String(
    parameters["rightBottomBlockAlign"] || "left"
  );
  ...
  const leftBottomBlockAlign = String(
    parameters["leftBottomBlockAlign"] || "left"
  );
  // 背景画像の設定
  ...
  Scene_MenuBase.prototype.createBackground = function () {
    ...
    if (originalBackgroundImage) {
      ...
      this._backgroundSprite.bitmap = ImageManager.loadSystem(
        originalBackgroundImage.imageName
      );
  ...
  Scene_MenuBase.prototype.calcMenuHelpWindowHeight = function () {
    ...
    let height = this.calcWindowHeight(1, false);
  ...
  Scene_Menu.prototype.commandWindowRect = function () {
    ...
    let wx = Graphics.boxWidth - ww;
    // let wy = this.mainAreaTop() + this.menuHelpWindowRect().height;
    let wy = 0 + this.menuHelpWindowRect().height;
    // 左側に配置
    if (commandWindowPosition === "left") {
      wx = 0;
    }
    // 上側に配置
    if (commandWindowPosition === "top") {
    ...
    // 下側に配置
    if (commandWindowPosition === "bottom") {
      ww = Graphics.boxWidth;
      wh = this.calcWindowHeight(2, true);
      wx = 0;
    ...
    if (
      ...
      (commandWindowPosition === "right" || commandWindowPosition === "left")
    ) {
  ...
  Scene_Menu.prototype.statusWindowRect = function () {
    // 右側に配置
    let ww = Graphics.boxWidth - this.mainCommandWidth();
    // let wh = this.mainAreaHeight() - this.menuHelpWindowRect().height;
    let wh = Graphics.boxHeight - this.menuHelpWindowRect().height;
    let wx = 0;
    // let wy = this.mainAreaTop() + this.menuHelpWindowRect().height;
    let wy = 0 + this.menuHelpWindowRect().height;
    // 左側に配置
    ...
    if (commandWindowPosition === "top") {
      ww = Graphics.boxWidth;
      // wy = this.mainAreaTop() + this.commandWindowRect().height;
      wy = 0 + this.commandWindowRect().height;
      // wh = this.mainAreaHeight() - this.commandWindowRect().height - this.menuHelpWindowRect().height;
      wh =
        Graphics.boxHeight -
    ...
    if (commandWindowPosition === "bottom") {
      ww = Graphics.boxWidth;
      // wy = this.mainAreaTop() + this.menuHelpWindowRect().height;
      wy = 0 + this.menuHelpWindowRect().height;
      // wh = this.mainAreaHeight() - this.commandWindowRect().height - this.menuHelpWindowRect().height;
      wh =
        Graphics.boxHeight -
        this.commandWindowRect().height -
  ...
  Scene_Menu.prototype.menuHelpWindowRect = function () {
    ...
    let wh = this.calcMenuHelpWindowHeight();
    // 上側に配置
    if (commandWindowPosition === "top") {
      wy = Graphics.boxHeight - wh;
    }
  ...
  Window_StatusBase.prototype.exDrawActorSimpleStatus = function (
    actor,
    x,
    y,
    width
    ...
    if (actorLevelLH > -1)
      this.drawActorLevel(
        actor,
        x + actorLevelX,
        y + lineHeight * actorLevelLH
      );
    if (actorIconLH > -1)
      this.drawActorIcons(
        actor,
        x + actorIconX,
        y + lineHeight * actorIconLH,
        width - actorIconX
      );
    if (actorClassLH > -1)
      this.drawActorClass(
  ...
  Window_MenuStatus.prototype.drawItemImage = function (index) {
    ...
    if (mPicture && showStandingPicture) {
      ...
      // ピンチ判定
      if (
        ExMenuScreenBase.getHpRate(actor.actorId()) >
  ...
  Window_MenuHelp.prototype.drawLeftBlock = function () {
    ...
    this.drawText(
      ...
      this.getFontSize() * leftBlockLabel.length,
      y,
      oneThirdWidth * 1 - this.getFontSize() * leftBlockLabel.length,
      leftBlockAlign
    );
  };
  ...
  Window_MenuHelp.prototype.drawRightBlock = function () {
    ...
    this.drawText(
      ...
      this.getFontSize() * rightBlockLabel.length + oneThirdWidth * 1,
      y,
      oneThirdWidth * 1 - this.getFontSize() * rightBlockLabel.length,
      rightBlockAlign
    );
  };
  ...
  Window_MenuHelp.prototype.drawCurrency = function () {
    ...
    if (currencyWindowPosition == "helpWindowRightBottom") {
      ...
      this.drawText(
        ...
        oneThirdWidth * 2,
        y,
        this.getFontSize() * rightBottomBlockLabel.length,
        rightBottomBlockAlign
      );
      this.resetTextColor();
      this.drawText(
        eval(rightBottomBlockValue),
        this.getFontSize() * rightBottomBlockLabel.length + oneThirdWidth * 2,
        y,
        oneThirdWidth * 1 - this.getFontSize() * rightBottomBlockLabel.length,
        rightBottomBlockAlign
      );
    }
  };
  ...
  Window_MenuHelp.prototype.drawMenuHelp = function () {
    ...
    if (leftBottomBlockLabel != "") {
      ...
      this.drawText(
        ...
        0,
        y,
        this.getFontSize() * leftBottomBlockLabel.length,
        leftBottomBlockAlign
      );
      this.resetTextColor();
      this.drawText(
        eval(leftBottomBlockValue),
        this.getFontSize() * leftBottomBlockLabel.length,
        y,
        oneThirdWidth * 1.5 - this.getFontSize() * leftBottomBlockLabel.length,
        leftBottomBlockAlign
      );
    } else {
      // メニューヘルプを表示
      ...
      this.drawText(
        ...
        0,
        y,
        this.innerWidth - oneThirdWidth * 1.5,
        leftBottomBlockAlign
      );
    }
  };
  ...
  Window_StatusBase.prototype.placeGauge = function (actor, type, x, y) {
    ...
    if (
      ...
      this.constructor.name === "Window_MenuActor"
    ) {
