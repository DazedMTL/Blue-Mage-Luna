...
(function () {
  ...
  const pEnemyShadowImage = setDefault(
    parameters["EnemyShadowImage"],
    "Shadow2"
  );
  const pEnemyShadowScaleX = setDefault(
    parameters["EnemyShadowScaleX"],
    "a.width / shadow.width * 1.5"
  );
  const pEnemyShadowScaleY = setDefault(
    parameters["EnemyShadowScaleY"],
    "a.width / shadow.width * 1.5"
  );
  const pEnemyShadowX = setDefault(parameters["EnemyShadowX"], 0);
  const pEnemyShadowY = setDefault(
    parameters["EnemyShadowY"],
    "(a.height / 20) * -1"
  );
  ...
  const pEnemyFloatAmplitude = setDefault(
    parameters["EnemyFloatAmplitude"],
    "5"
  );
  const pEnemyFloatPeriodicTime = setDefault(
    parameters["EnemyFloatPeriodicTime"],
    "120"
  );
  // アクター
  ...
  const pActorFloatAmplitude = setDefault(
    parameters["ActorFloatAmplitude"],
    "5"
  );
  const pActorFloatPeriodicTime = setDefault(
    parameters["ActorFloatPeriodicTime"],
    "120"
  );
  // 共通
  const pRandomStartFloatHeight = toBoolean(
    parameters["RandomStartFloatHeight"],
    true
  );
  const pTransparencyJumpHeight = toNumber(
    parameters["TransparencyJumpHeight"]
  );
  ...
  function getDynamicMotionParameters() {
    const isDynamicMotionMZ = PluginManager._scripts.some(function (
      scriptName
    ) {
   ...
   * ●アクターの影作成
   */
  const _Sprite_Actor_createShadowSprite =
   ...
   * ●更新時の位置補正
   */
  const _Sprite_Battler_updatePosition =
  ...
  Sprite_Battler.prototype.updatePosition = function () {
    ...
    if (this.floatHeight) {
      // 変動幅を設定し、Ｙ座標に加算
      this.floatSwing =
        Math.sin((this._floatTime / this._floatPeriodicTime) * Math.PI * 2) *
        this._floatAmplitude;
