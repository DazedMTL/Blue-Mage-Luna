  ...
  Sprite_Enemy.prototype.setBigEnemy = function () {
    ...
    if (this._bigEnemy === "UnderPosition") {
      ...
      if (this._bigEnemy === "OriginalRatio") {
        ...
        height = Math.floor(
          (Graphics.height - bitmap.height * this.scale.y) / 2
        );
      } else {
        this.scale.y =
          (Graphics.height / bitmap.height) * (correctionScale || 1);
      }
      this._originalscaleY = this.scale.y;
      this._homeY =
        Math.floor(
          (Graphics.height - Graphics.boxHeight) / 2 + Graphics.boxHeight
        ) +
        24 -
        height;
    }
    this._homeY += this._enemy.enemy().meta.BigEnemyY
      ? Number(this._enemy.enemy().meta.BigEnemyY)
      : 0;
    this.updatePosition();
  };
