...
(() => {
  const PNAME = "MKR_PlayerSensorMZ";
  ...
  function CEC_float(text, def, min, max) {
    return (
      isFinite(text) ? parseFloat(text) : isFinite(def) ? parseFloat(def) : 0
    ).clamp(min, max);
  }
  ...
  const convertEscapeCharacters = function (text) {
    ...
    if (scene && scene._windowLayer) {
      ...
      return windowChild ? windowChild.convertEscapeCharacters(text) : text;
    } else {
      return ConvVb(text);
    }
  };
  ...
  let DIR_UP,
    ...
    DefFollowerThrough,
    DefLocationReset;
  DefSensorSwitch = CheckParam(
    "switch",
    "Sensor_Switch",
    Parameters["Sensor_Switch"],
    "D"
  );
  DefLostSensorSwitch = CheckParam(
    "switch",
    "Lost_Sensor_Switch",
    Parameters["Lost_Sensor_Switch"]
  );
  DefBothSensor = CheckParam(
    "bool",
    "Both_Sensor",
    Parameters["Both_Sensor"],
    false
  );
  DefRangeVisible = CheckParam(
    "bool",
    "Range_Visible",
    Parameters["Range_Visible"],
    true
  );
  DefTerrainDecision = CheckParam(
    "bool",
    "Terrain_Decision",
    Parameters["Terrain_Decision"],
    false
  );
  DefRangeColor = CheckParam(
    "string",
    "Range_Color",
    Parameters["Range_Color"],
    "white"
  );
  DefRangeOpacity = CheckParam(
    "num",
    "Range_Opacity",
  ...
  DefRangePosition = CheckParam(
    "num",
    "Range_Position",
    Parameters["Range_Position"],
    1
  );
  DefAutoSensor = CheckParam(
    "bool",
    "Auto_Sensor",
    Parameters["Auto_Sensor"],
    false
  );
  DefEventDecision = CheckParam(
    "bool",
    "Event_Decision",
    Parameters["Event_Decision"],
    false
  );
  DefRegionDecisions = [];
  ...
  DefRealRangeX = CheckParam(
    "float",
    "Real_Range_X",
    Parameters["Real_Range_X"],
    0.0,
    0.0,
    0.999
  );
  DefRealRangeY = CheckParam(
    "float",
    "Real_Range_Y",
    Parameters["Real_Range_Y"],
    0.0,
    0.0,
    0.999
  );
  DefFoundBallon = CheckParam(
    "num",
    "Player_Found.Ballon",
    Parameters["Player_Found"]["Ballon"],
    0,
    0
  );
  DefFoundCommon = CheckParam(
    "num",
    "Player_Found.Common_Event",
    Parameters["Player_Found"]["Common_Event"],
    0,
    0
  );
  DefFoundDelay = CheckParam(
    "num",
    "Player_Found.Delay",
    Parameters["Player_Found"]["Delay"],
    0,
    0
  );
  DefFoundSe = {
    name: CheckParam(
      ...
      Parameters["Player_Found"]["Se"]["Name"],
      ""
    )[0],
    volume: CheckParam(
      "num",
      "Player_Found.Se.Volume",
      Parameters["Player_Found"]["Se"]["Volume"],
      90,
      0,
      100
    )[0],
    pitch: CheckParam(
      ...
      Parameters["Player_Found"]["Se"]["Pitch"],
      100,
      50,
      150
    )[0],
    pan: CheckParam(
      "num",
      "Player_Found.Se.Pan",
      Parameters["Player_Found"]["Se"]["Pan"],
      0,
      -100,
      100
    )[0],
  };
  DefLostBallon = CheckParam(
    "num",
    "Player_Lost.Ballon",
    Parameters["Player_Lost"]["Ballon"],
    0,
    0
  );
  DefLostCommon = CheckParam(
    "num",
    "Player_Lost.Common_Event",
    Parameters["Player_Lost"]["Common_Event"],
    0,
    0
  );
  DefLostDelay = CheckParam(
    "num",
    "Player_Lost.Delay",
    Parameters["Player_Lost"]["Delay"],
    0,
    0
  );
  DefLostSe = {
    name: CheckParam(
      "string",
      "Player_Lost.Se.Name",
      Parameters["Player_Lost"]["Se"]["Name"],
      ""
    )[0],
    volume: CheckParam(
      "num",
      "Player_Lost.Se.Volume",
      Parameters["Player_Lost"]["Se"]["Volume"],
      90,
      0,
      100
    )[0],
    pitch: CheckParam(
      ...
      Parameters["Player_Lost"]["Se"]["Pitch"],
      100,
      50,
      150
    )[0],
    pan: CheckParam(
      "num",
      "Player_Lost.Se.Pan",
      Parameters["Player_Lost"]["Se"]["Pan"],
      0,
      -100,
      100
    )[0],
  };
  DefTrackingPriority = CheckParam(
    "bool",
    "Tracking_Priority",
    Parameters["Tracking_Priority"],
    false
  );
  DefFollowerThrough = CheckParam(
    "bool",
    "Follower_Through",
    Parameters["Follower_Through"],
    false
  );
  DefLocationReset = CheckParam(
    "bool",
    "Location_Reset",
    Parameters["Location_Reset"],
    false
  );
  ...
  //    2 = 即ロスト
  //=========================================================================
  const _Game_CharacterBaseInitMembers =
  ...
  const _Game_CharacterBaseMoveStraight =
  ...
  const _Game_CharacterBaseMoveDiagonally =
  ...
  const _Game_CharacterBaseSetDirection =
  ...
  Game_Event.prototype.foundPlayer = function () {
    ...
    if (delay <= 0) {
      ...
      // 発見後スイッチON
      const sw_on =
        this.getSensorSwitch() !== null ? this.getSensorSwitch() : sensorSwitch;
      foundPlayer_swon(sw_on, mapId, eventId);
      // ロスト後スイッチOFF
      const sw_off =
        this.getLostSensorSwitch() !== null
          ? this.getLostSensorSwitch()
          : lostSensorSwitch;
      if (sw_off !== "") {
  ...
  Game_Event.prototype.sensorSquare = function () {
    ...
    // プレイヤー範囲探索
    if (
      Math.abs(sx) <= sensorRange + realX &&
      Math.abs(sy) <= sensorRange + realY
    ) {
      return true;
    }
  };
  ...
  const _Game_Event_isCollidedWithEvents =
    Game_Event.prototype.isCollidedWithEvents;
  Game_Event.prototype.isCollidedWithEvents = function (x, y) {
    if (this.isSensorFound() && DefTrackingPriority[0]) {
      return Game_CharacterBase.prototype.isCollidedWithEvents.apply(
        this,
        arguments
      );
    } else {
  ...
  const _Game_Event_isCollidedWithPlayerCharacters =
    ...
    if (direction === DIR_UP) {
      ...
      if (coordinates && cnt > 0) {
        ...
        for (let i = 1, j = 2; j < cnt; i++, j++) {
          ...
          cy -= tileHeight / 2;
          distanceX =
            width - tileWidth / 2 - Math.abs(coordinates[j][num]) * tileWidth;
          distanceY = cy + tileHeight * signChange(sign);
          this.mkrDrawLine(context, cx, cy, distanceX, distanceY);
        }
      }
  ...
  Bitmap.prototype.mkrDrawLine = function (
    context,
    cx,
    cy,
    distanceX,
