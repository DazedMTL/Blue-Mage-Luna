 ...
 * @plugindesc Effekseer Extended plugin v1.70 - 1.04
 * @author Effekseer
 *
 * @help
 ...
 * @target MZ
 * @url https://github.com/effekseer/EffekseerForRPGMakerMZ_Ex
 * @plugindesc Effekseer 拡張プラグイン v1.70 - 1.04
 * @author Effekseer
 *
 * @help
...
(() => {
  ...
  var paramInstanceMaxCount = Number(
    PluginManager.parameters(pluginName)["InstanceMaxCount"]
  );
  var paramSquareMaxCount = Number(
    PluginManager.parameters(pluginName)["SquareMaxCount"]
  );
  var isDistortionEnabled =
    PluginManager.parameters(pluginName)["DistortionEnabled"] != "false";

  Graphics._createEffekseerContext = function () {
    if (this._app && window.effekseer) {
      try {
        const actualInstanceMaxCount = paramInstanceMaxCount
          ? paramInstanceMaxCount
          : 10000;
        const actualSquareMaxCount = paramSquareMaxCount
          ? paramSquareMaxCount
          : 10000;
  ...
  if (isDistortionEnabled) {
    Sprite_Animation.prototype.setProjectionMatrix = function (renderer) {
      const x =
        ((this._mirror ? -1 : 1) * renderer.view.height) / renderer.view.width;
      const y = -1;
      // prettier-ignore
    ...
    Sprite_Animation.prototype._render = function (renderer) {
      ...
      if (this._targets.length > 0 && this._handle && this._handle.exists) {
        ...
        if (!this.isBackgroundCaptured) {
          Graphics.effekseer.captureBackground(
            0,
            0,
            renderer.view.width,
