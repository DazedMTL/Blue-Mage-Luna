...
(() => {
  ...
  const PlaybackRateHPRecoveryText = String(
    parameters["PlaybackRateHPRecoveryText"] || "%1のHPが%2回復した！"
  );
  const PlaybackRateHPDamageText = String(
    parameters["PlaybackRateHPDamageText"] || "%1のHPが%2減った！"
  );
  const PlaybackRateMPRecoveryText = String(
    parameters["PlaybackRateMPRecoveryText"] || "%1のMPが%2回復した！"
  );
  const PlaybackRateMPDamageText = String(
    parameters["PlaybackRateMPDamageText"] || "%1のMPが%2減った！"
  );
  const PlaybackRateTPRecoveryText = String(
    parameters["PlaybackRateTPRecoveryText"] || "%1のTPが%2溜まった！"
  );
  const PlaybackRateTPDamageText = String(
    parameters["PlaybackRateTPDamageText"] || "%1のTPが%2減った！"
  );

  const _Window_BattleLog_displayRegeneration =
  ...
  Window_BattleLog.prototype.displayPlaybackRate = function (subject) {
    if (subject.shouldPopupDamage()) {
      ...
      if (result.hpDamage < 0) {
        text = PlaybackRateHPRecoveryText.format(
          subject.name(),
          result.hpDamage
        );
      ...
      if (result.mpDamage < 0) {
        text = PlaybackRateMPRecoveryText.format(
          subject.name(),
          result.mpDamage
        );
        this.push("addText", text);
      ...
      if (result.tpDamage < 0) {
        text = PlaybackRateTPRecoveryText.format(
          subject.name(),
          result.tpDamage
        );
        this.push("addText", text);
